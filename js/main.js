window.addEventListener("DOMContentLoaded", function (e) {
  ...
  function handlePresentationClick(e) {
    ...
    if (next) {
      ...
      var autoAdvanceEl = parseInt(next.getAttribute("data-autoadvance"));
  
      if (!isNaN(autoAdvanceEl)) {
        setTimeout(function (e) {
          handlePresentationClick(e);
        }, autoAdvanceEl);
      }
  
      var onShowAttr = next.getAttribute("data-onshow");
      if (onShowAttr) {
        window[onShowAttr]();
      }
    }
  ...
  function handleAnimationEnd(e) {
    ...
    var autoAdvanceEl = slide.getAttribute("data-autoadvance");
  
    if (autoAdvanceEl == "animationend" && slide.classList.contains("active")) {
      handlePresentationClick(e);
    }
  }
  ...
  function showLearning() {
    var indexedImage = Math.floor(Math.random() * shapes.length);
    setLearnImage(shapes[indexedImage]);
  
    var slide = document.querySelector("hp-slide.active");
    slide.classList.remove("learn-yes");
    slide.classList.remove("learn-no");
    slide.classList.add(indexedImage ? "learn-no" : "learn-yes");
  }
